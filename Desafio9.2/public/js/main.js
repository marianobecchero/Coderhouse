...
// compruebo si existe alguna sesión iniciada
const login = async () => {
  ...
  if (res.state === 'on') {
    document.getElementById('logout').style.display = "none"
      manejarEventoSession(res.nombre)
  } else {
      // si no existe redirijo a la página de login
...
async function manejarEventoSession(nombre) {
  ...
  // relleno la plantilla con las personas recibidas
  const html = functionTemplate({ nombre })

  // reemplazo el contenido del navegador con los nuevos datos
  document.getElementById('session').innerHTML = html

  // guardo el nombre de usuario para el saludo cuando cierre sesión
...
formSession.addEventListener('submit', e => {
  ...
  const logout = async() => {
    ...
    //cierro la sesión
      const request = await fetch('http://localhost:8080/api/sesiones/logout')
      const res = await request.json()

      document.getElementById('contenedor').style.display = "none"
      document.getElementById('logout').style.display = "block"
      document.getElementById('saludo').innerHTML = `Hasta luego ${nombreusuario}`
      ...
      //window.location.href = "http://localhost:8080/login/logout.html"
  }
  logout()
})
