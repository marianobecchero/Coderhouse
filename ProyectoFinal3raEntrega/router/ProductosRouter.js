...
productosRouter.get('/:id', async (req, res) => {
    const execute = async() => {
        ...
        if (isNaN(id)){
            ...
            if (producto == null){
                res.json({ message: `No se encontró el producto con id ${id}`})
            } else {
                res.json(producto)
            }
        }
...
productosRouter.post('/', async (req, res) => {
    const execute = async() => {
        if (esAdministrador == true){
            ...
            res.json({ message: 'El producto se agregó correctamente' })
        } else {
            res.json({ message: 'No tiene permisos para realizar este método' })
        }  
    }
    execute()
})
        ...
        if (esAdministrador == true){
            ...
            if(isNaN(idProducto)){
                ...
                if (producto == null){
                    ...
                    res.json({ message: 'El producto se modificó correctamente' })
                }
            }
        } else {
            res.json({ message: 'No tiene permisos para realizar este método' })
